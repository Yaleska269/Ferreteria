...
public class DetallesVentasControlador {
    ...
    private final DetallesVentasDAO detallesVentasDAO;

    public DetallesVentasControlador() {
        this.detallesVentasDAO = new DetallesVentasDAO();
    }
    ...
    public void crearDetalleVenta(int idVenta, int idProducto, int cantidad, float precioUnitario) {
        try {
            ...
            detallesVentasDAO.crearDetalleVenta(detalle);
    ...
    public List<DetallesVentas> obtenerTodosDetallesVenta() {
        try {
            return detallesVentasDAO.leerTodosDetallesVentas();
        } catch (SQLException e) {
    ...
    public void actualizarDetalleVenta(int idDetalleVenta, int idVenta, int idProducto, int cantidad, float precioUnitario) {
        try {
            ...
            detalle.setCantidad(cantidad);
            detalle.setPrecioUnitario(precioUnitario);
            detallesVentasDAO.actualizarDetalleVenta(detalle);
    ...
    public void eliminarDetalleVenta(int idDetalleVenta) {
        try {
            detallesVentasDAO.eliminarDetalleVenta(idDetalleVenta);
    ...
    public static void main(String[] args) {
        DetallesVentasControlador controlador = new DetallesVentasControlador();

        // Crear un detalle de venta
        controlador.crearDetalleVenta(1, 4, 44, 33.22f);

        // Leer todos los detalles de venta
        List<DetallesVentas> detalles = controlador.obtenerTodosDetallesVenta();
        if (detalles != null) {
